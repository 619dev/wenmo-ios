...
final class KeyboardViewController: UIInputViewController {
    ...
    private func rebuildKeyboard() {
        ...
        for (index, keys) in rows.enumerated() { rootStack.addArrangedSubview(makeRow(keys, inset: index == 1 ? 14 : (index == 2 ? 35 : 0))) }
        rootStack.addArrangedSubview(makeBottomRow())
        refreshCandidates()
    }

    private func addCandidateBar() {
        candidateScroll.backgroundColor = UIColor(white: 0.96, alpha: 1)
    ...
    private func makeRow(_ titles: [String], inset: CGFloat) -> UIView {
        ...
        let left = UIView(); let right = UIView()
        if inset > 0 {
            left.widthAnchor.constraint(equalToConstant: inset).isActive = true
            right.widthAnchor.constraint(equalToConstant: inset).isActive = true
        ...
        if inset > 0 { row.addArrangedSubview(right) }
        return row
    }
    ...
    private func keyButton(_ title: String, dark: Bool = false) -> UIButton {
        ...
        button.titleLabel?.font = .systemFont(ofSize: title.count == 1 ? 20 : 15)
        button.backgroundColor = dark ? UIColor(white: 0.68, alpha: 1) : .systemBackground
    ...
    private func refreshCandidates() {
        ...
        compositionLabel.textColor = engine.composition.isEmpty ? .secondaryLabel : .label
